...
*/
	public function OnAddContentCaptcha(&$content)
	{
		if (!defined("ADMIN_SECTION")){
			...
			if($arSettings["act"] == "Y" && self::checkBrowser() && !self::checkMask($arSettings["mask_exclusion"])){
				...
				$replace = array (
					'<div class="g-recaptcha" data-theme="' . $theme . '" data-sitekey="'. $arSettings["key"] .'"  data-size="'. $size .'"  data-badge="'. $badge .'" data-callback="RecaptchaSubmitForm"></div>',
					'<div class="g-recaptcha" data-theme="' . $theme . '" data-sitekey="'. $arSettings["key"] .'" data-size="'. $size .'"  data-badge="'. $badge .'" data-callback="RecaptchaSubmitForm"></div>',
	...
	public function OnVerificContent(){
        ...
        if($arSettings["act"] !== "Y" && !self::checkBrowser()){return;}
            Asset::getInstance()->addJs('/bitrix/js/uni.recaptcha_2/script.js');
            Asset::getInstance()->addJs('https://www.google.com/recaptcha/api.js?onload=onloadRecaptcha&render=explicit&hl='. LANGUAGE_ID);
	...
	private function getParamSite(){
		$settings = COption::GetOptionString("uni.recaptcha_2", "settings", false, SITE_ID);
